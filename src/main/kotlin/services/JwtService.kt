...
class JwtService(private val jwtSecret: String) {
    ...
    private val issuer = "study-session-auth-service"
    private val audience = "study-session-app"
    ...
    fun generateToken(user: User): TokenPayload {
        ...
        val token = JWT.create().withAudience(audience).withIssuer(issuer).withClaim("id", user.id)
            ...
            .withExpiresAt(expiration) // Token expires in 10 minutes
    ...
    fun verifyToken(token: String): User? {
        val verifier = JWT.require(Algorithm.HMAC256(jwtSecret)).withAudience(audience).withIssuer(issuer).build()
